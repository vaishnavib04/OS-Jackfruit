...
struct monitored_entry {
    ...
    int soft_limit_warned;       /* 1 if soft warning already emitted */
 ...
 *   - the timer callback runs in softirq context on older kernels
 *     but mod_timer + del_timer_sync here run from process context
 *   - get_rss_bytes() calls get_task_mm / mmput which can sleep,
 *     so we must NOT hold a spinlock around those calls
 *   - a mutex is safe for all three code paths: timer, ioctl register,
 *     ioctl unregister, and module exit
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * list_for_each_entry_safe allows us to delete the current entry
     * during iteration without corrupting the list traversal.
     */
